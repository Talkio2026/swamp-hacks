        ...
        <div className="features-grid mt-14 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {FEATURES.map(({ title, body, icon }) => (
            <article
              key={title}
              className={cn(
                'feature-card group flex flex-col rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-6 shadow-lg transition-all duration-300',
                'hover:border-white/20 hover:bg-white/10 hover:shadow-xl hover:-translate-y-1'
              )}
            >
              <div className="mb-4 flex items-start justify-between">
                <span className="rounded-lg bg-white/10 p-2 text-white">{icon}</span>
              </div>
