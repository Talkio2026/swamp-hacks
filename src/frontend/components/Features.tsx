        ...
        <div className="features-grid mt-14 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {FEATURES.map(({ title, body, icon }) => (
            <article
              key={title}
              className={cn(
                'feature-card group flex flex-col rounded-2xl border border-white/10 bg-white/5 p-6 transition-all duration-200',
                'hover:border-white/20 hover:bg-white/[0.07] focus-within:ring-2 focus-within:ring-white/30 focus-within:ring-offset-2 focus-within:ring-offset-black'
              )}
            >
              ...
              <p className="mt-2 flex-1 text-sm text-slate-400">{body}</p>
            </article>
          ))}
        </div>
      </div>
