...
export function Features() {
  ...
  return (
    <section
      ref={sectionRef}
      id="features"
      className="relative pt-24 pb-14 px-6 lg:px-8 lg:pt-32 lg:pb-20"
    ...
    >
      <div className="mx-auto max-w-[1280px]">
        ...
        <p className="mt-4 text-center text-sm text-slate-300 max-w-3xl mx-auto">
        ...
        <div className="features-grid mt-14 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {FEATURES.map(({ title, body, icon }) => (
            <article
              key={title}
              className={cn(
                'feature-card group flex flex-col rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-6 transition-all duration-200',
                'hover:border-white/20 hover:bg-white/10 focus-within:ring-2 focus-within:ring-white/30 focus-within:ring-offset-2 focus-within:ring-offset-[#162d3d]'
              )}
            >
              ...
              <p className="mt-2 flex-1 text-sm text-slate-300">{body}</p>
            </article>
          ))}
        </div>
      </div>
